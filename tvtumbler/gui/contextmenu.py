...
This is robbed from script.globalsearch (with some changes), which in turn credits this to 'Nuka1195'.
So, our thanks and appreciation go to Nuka1195.

<not really sure to do as regards copyright here>
'''
...
class ContextMenuDialog(xbmcgui.WindowXMLDialog):
    ...
    @classmethod
    def select(cls, options):
        assert len(options) < 5, 'A maximum of 4 context menu options is currently permitted.'
        dlg = cls("script-tvtumbler-contextmenu.xml", __addonpath__, "Default",
                  area=(0, 0, 1280, 720,), labels=options)
        dlg.doModal()
        sel = dlg.selection
        del dlg
        return sel
